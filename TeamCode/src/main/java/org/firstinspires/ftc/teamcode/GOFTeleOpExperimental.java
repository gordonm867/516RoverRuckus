...
@TeleOp(name="GOFFastTeleOp", group="GOF")
public class GOFTeleOpExperimental extends OpMode {

    private volatile    boolean             aPressed            = false;
    private volatile    boolean             doTelemetry         = true;
    private volatile    boolean             ypressed            = false;
    private volatile    boolean             bumperPressed       = false;
    private volatile    boolean             servoMove           = false;

    private volatile    double              angle;
    private volatile    double              drive;
    private volatile    double              firstAngleOffset;
    private volatile    double              lastIntake          = 0;
    private volatile    double              maxDriveSpeed;
    private volatile    double              turn;

    private volatile    ElapsedTime         elapsedTime         = new ElapsedTime();

    public  volatile    GOFHardware         robot               = GOFHardware.getInstance(); // Use the GOFHardware class

    private volatile    int                 driverMode          = 1;

    private             Thread              update;
    private             Thread              driveBot;
    private             Thread              scoreBot;
    private             Thread              intake;
    private             Thread              extendFlip;
